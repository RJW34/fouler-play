...
def test_runtime_ready_status_clears_stale_blocker_truth(tmp_path, monkeypatch):
    monkeypatch.setattr(state_store, "ACTIVE_BATTLES_PATH", tmp_path / "active_battles.json")
    monkeypatch.setattr(state_store, "STREAM_STATUS_PATH", tmp_path / "stream_status.json")
    monkeypatch.setattr(state_store, "DAILY_STATS_PATH", tmp_path / "daily_stats.json")
    monkeypatch.setattr(state_store, "STABILITY_REPORT_PATH", tmp_path / "stability_report.json")

    state_store.write_runtime_blocked_status(
        code="showdown_credential_rejected",
        summary="Showdown login failed; credential was rejected.",
    )
    payload = state_store.write_runtime_ready_status(
        mode="login_proven",
    ...
    assert active["runtime_blocked"] is False
    assert active["runtime_mode"] == "login_proven"
    assert status["status"] == "Ready"
    assert status["runtime_mode"] == "login_proven"
    assert status["wins"] == daily["wins"]
    assert status["losses"] == daily["losses"]
    assert "blocker_code" not in status
    assert payload["stabilityReport"]["runtime_blocked"] is False
    assert payload["stabilityReport"]["stability"]["health"] == "ready"
