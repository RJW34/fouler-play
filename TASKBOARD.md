...
reports were publishing false last-five windows and unsupported filler text; see
`reports/codex_fouler_reporting_fix_20260619.md`.

Current live requirements:

- Battle supervisor command includes `--skip-improve` by default.
- `cleanup-stale-truth` is allowed by the active proof-window runtime lease.
- Timeout/disconnect outcomes must be reported as losses, never ties.
- Discord battle reports must compute recent windows with the just-finished
  battle included and must not invent strategic cause text without replay-backed
  evidence.
...
> Superseded 2026-06-20 21:06 UTC: JIGGLYPUFF is currently reachable at the live Fouler runtime, `devstream\truth\runtime-lease.json` names account `LEBOTJAMESXD00N`, and `devstream\truth\latest-elo-proof.json` is regenerated by `scripts\fouler_mission_monitor.py --write` from current `battle_stats.json`/health truth. Latest proof at that time: `battle-gen9ou-2636033915`, final rating `1236`, recent 5 `4-1`, recent 20 `10-10`. Treat the bullets below as historical 2026-06-15 recovery context, not current live truth.
...
- **Historical stale note:** the 2026-06-15 "no post-start ladder proof yet" condition is no longer current; use `devstream\truth\latest-elo-proof.json` and `battle_stats.json` for current proof.
