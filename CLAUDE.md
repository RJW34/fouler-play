...
1. **Phase 2-4 roadmap items** — see TASKBOARD.md. Bayesian set inference, switch prediction, archetype classification. Implement based on current ELO bracket. Run `python -m pytest tests/ -v` after each change.
2. **Replay analysis improvements** — `replay_analysis/` is BUILT and working (`team_performance.py`, `analyzer.py`, `turn_review.py`). Improve the analysis, don't rebuild from scratch. The developer loop (`infrastructure/linux/analyze_performance.sh`) already feeds reports to Claude Code.
...
- Agent 1: Implement a Phase 2-4 improvement
- Agent 2: Analyze replay data for weakness patterns
...
2. **OBS + Twitch streaming** — you own the entire streaming pipeline:
   - `streaming/` is **BUILT and working** (23 files). Do NOT rebuild it from scratch.
   - **Key files** (these are the ones that matter — ignore/delete the rest):
     - `serve_obs_page.py` (820 lines): Main HTTP + WebSocket server, OBS-websocket integration, ELO polling. Start with `python streaming/serve_obs_page.py`. Runs on port 8777.
     - `obs_websocket.py`: OBS WebSocket client for controlling Browser Source URLs.
     - `state_store.py`: Reads/writes `active_battles.json` and `stream_status.json`.
     - `obs_controller.py`: Higher-level OBS control.
     - `overlay.html`: Stats overlay (ELO, win/loss, team info).
     - `obs_battles.html`: Battle display page.
   - **Dead code to ignore**: `auto_stream.py`, `auto_stream_v2.py`, `auto_stream_firefox.py`, `auto_stream_headless.py`, `auto_stream_stable.py` — these are abandoned experiments from a previous bot account. They reference the wrong username ("LEBOTJAMESXD005"). Do not use or fix them.
   - **How the streaming pipeline works**:
     1. Bot plays games → `fp/run_battle.py` writes `active_battles.json` (via `update_active_battles_file()`)
     2. `streaming/serve_obs_page.py` reads `active_battles.json` and serves it over HTTP + WebSocket
     3. OBS Browser Sources connect to `http://localhost:8777/obs` (battle display) and `http://localhost:8777/overlay` (stats)
     4. obs-websocket (port 4455) can also directly set Browser Source URLs to live battle pages
...
- Agent 3: Diagnose and fix any streaming/bot issues
...
├── streaming/                # OBS/Twitch integration (BUILT — see BAKUGO responsibilities)
├── replay_analysis/          # Performance analysis (BUILT — team_performance.py, analyzer.py, turn_review.py)
