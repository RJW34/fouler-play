...
async def _post_battle_to_discord(
    ...
    is_win = _is_our_showdown_account(winner, our_player_name)
...
def _hp_fraction_sum(battler) -> float:
    total = 0.0
    pkmn = list(getattr(battler, "reserve", []) or [])
...
def score_on_cap(battle) -> dict:
    ...
    if my < opp:
        ...
        we_forfeit = str(getattr(battle.user, "name", "p1")) > str(getattr(battle.opponent, "name", "p2"))
    ...
    try:
        while True:
            ...
            if battle_is_finished(battle_tag, msg):
                ...
                try:
                    is_win = _is_our_showdown_account(
                        winner,
                        getattr(getattr(battle, "user", None), "account_name", None),
                    )
                ...
                try:
                    _we_won_ev = _is_our_showdown_account(
                        winner,
                        getattr(getattr(battle, "user", None), "account_name", None),
                    )
            ...
            if action_required and not battle.wait:
                _turn_cap = getattr(FoulPlayConfig, "battle_turn_cap", 0) or 0
                if _turn_cap > 0 and isinstance(battle.turn, int) and battle.turn >= _turn_cap:
                    _dec = score_on_cap(battle)
                    if _dec["we_forfeit"]:
                        logger.info("Turn cap %d reached at turn %s; forfeiting (my=%.3f < opp=%.3f) -- score-on-cap", _turn_cap, battle.turn, _dec["my_hp_sum"], _dec["opp_hp_sum"])
                        await ps_websocket_client.send_message(battle.battle_tag, ["/forfeit"])
                        continue
                    else:
                        logger.info("Turn cap %d reached at turn %s; holding for opponent forfeit (my=%.3f >= opp=%.3f)", _turn_cap, battle.turn, _dec["my_hp_sum"], _dec["opp_hp_sum"])
