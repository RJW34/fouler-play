...
def _hp_fraction_sum(battler) -> float:
    """Sum of hp/max_hp across a battler's whole team (fainted counts as 0).

    This is the dominant term of any reasonable battle evaluation and is the
    score-on-cap heuristic the task calls for: at the turn cap, the side with
    the higher remaining HP fraction is judged the (provisional) winner. It is
    cheap, deterministic, and needs no poke-engine state conversion.
    """
    total = 0.0
    pkmn = list(battler.reserve)
    if battler.active is not None:
        pkmn.append(battler.active)
    for p in pkmn:
        if p is None:
            continue
        mx = getattr(p, "max_hp", 0) or 0
...
def score_on_cap(battle) -> dict:
    """Decide, at the turn cap, whether *we* should forfeit (score-on-cap).

    Returns a dict with the two HP-fraction sums and a boolean ``we_forfeit``.
    The rule must be one BOTH self-play engines independently agree on so that
    EXACTLY one side forfeits (otherwise a tie could strand the battle): the
    side with the strictly-lower HP-fraction sum forfeits; on an exact tie the
    player whose slot id sorts later (``p2`` > ``p1``) forfeits. This makes a
    capped battle terminate DECISIVELY -- Showdown emits a real ``|win|`` line
    for the higher-HP side, so the eval counts it as decisive (not discarded).
    """
    my = _hp_fraction_sum(battle.user)
    opp = _hp_fraction_sum(battle.opponent)
    if my < opp:
        we_forfeit = True
    elif my > opp:
        we_forfeit = False
    else:
        # Exact tie: deterministic, side-symmetric tiebreak on the slot id so
        # the two engines never both forfeit or both stay.
        we_forfeit = str(getattr(battle.user, "name", "p1")) > str(
            getattr(battle.opponent, "name", "p2")
        )
    return {"my_hp_sum": my, "opp_hp_sum": opp, "we_forfeit": we_forfeit}
    ...
    while True:
        ...
        if battle_is_finished(battle.battle_tag, msg):
            ...
            if action_required and not battle.wait:
                turn_cap = getattr(FoulPlayConfig, "battle_turn_cap", 0) or 0
                if (
                    turn_cap > 0
                    and isinstance(battle.turn, int)
                    and battle.turn >= turn_cap
                ):
                    decision = score_on_cap(battle)
                    if decision["we_forfeit"]:
                        logger.info(
                            "Turn cap %d reached at turn %s; forfeiting "
                            "(my_hp_sum=%.3f < opp_hp_sum=%.3f) -- score-on-cap",
                            turn_cap, battle.turn,
                            decision["my_hp_sum"], decision["opp_hp_sum"],
                        )
                        await ps_websocket_client.send_message(
                            battle.battle_tag, ["/forfeit"]
                        )
                        # Stop choosing moves; the |win| for the opponent will
                        # arrive on the next receive and end the battle decisively.
                        continue
                    else:
                        logger.info(
                            "Turn cap %d reached at turn %s; holding for "
                            "opponent forfeit (my_hp_sum=%.3f >= opp_hp_sum=%.3f)",
                            turn_cap, battle.turn,
                            decision["my_hp_sum"], decision["opp_hp_sum"],
                        )
                best_move = await async_pick_move(battle)
