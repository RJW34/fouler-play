...
# --- Ported clock-safety fix (fork commits bb9ba617 + 363a934-era parser) ---
# battle.time_remaining must track the CUMULATIVE side clock ("N sec total"),
# not the per-turn grace, and must keep draining once Showdown switches from
# "Time left:" messages to "<user> has N seconds left" countdown warnings.
# Ignoring the countdown left time_remaining stale-HIGH while the real bank
# drained to zero -> mass inactivity forfeits (the 2026-06-27 ELO collapse).
...
def _parse_time_left_seconds(text: str) -> int | None:
    ...
    # that bleeds to zero and forfeits the game. Prefer it when present; fall
    # back to the first "N sec" figure (upstream behavior) otherwise.
...
def inactive(battle, split_msg):
    if len(split_msg) < 3:
        return
    # Re-join the protocol remainder: Showdown's full clock message is
    # "Time left: N sec this turn | M sec total" and the "|" split scatters
    # the cumulative total into split_msg[3+].
    text = "|".join(str(s) for s in split_msg[2:])

    if text.startswith(constants.TIME_LEFT):
    ...
    # "<user> has N seconds left" countdown warnings (30/20/15/10/5). Consume our own countdown
    # so every time-budget safeguard keeps firing all the way down (fork fix bb9ba617, issue #79).
